...
class PracticeList(APIView):
    def get(self, request, **kwargs):
        ...
        search_title_and_nickname = request.GET.get('search-title-and-nickname')
        ...
        # 제목 + 닉네임검색
        elif search_title_and_nickname:
            queryset = Practice.objects.filter(
                Q(title__icontains=search_title_and_nickname) |
                Q(user__profile__nickname__icontains=search_title_and_nickname)
                ).order_by('-created_at')
